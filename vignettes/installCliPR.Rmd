...
As Herper is not public yet you will need to add a GITHUB_PAT to your environment to install. Please contact [Kathryn Rozen-Gagnon](mailto:krozen@rockfeller.edu) for access... This will not be necessary once Herper becomes public.

Once access is provided, you can generate your GITHUB pat by visiting the settings -> Developer Settings ->  Personal access tokens and creating a personal access token. Copy the long string of your personal access token replace **MYGITHUBPAT** below.
...
```{r installHerper,echo=TRUE,eval=FALSE}
library(devtools)
