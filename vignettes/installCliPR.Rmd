---
title: "Using and installing the CTK pipeline using ClipR"
author: "Kathryn Rozen-Gagnon, Ji-Dung Luo and Thomas Carroll"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{install_clipR}
...
The ClipR package provides a set of its functions for the processing and analysis of differing forms of ClIP data for use within the R framework.

For more information on the processing of data using ClipR you can see our [quickstart](StandardandBrdU_Processing_CTK_small.html) and full [vignettes](StandardandBrdU_Processing_CTK.html).
...
As clipr is not public yet you will need add a GITHUB_PAT to your environment to install. 
...
We can install ClipR package from Github using the devtools package

```{r installClipR,echo=TRUE,eval=FALSE}
install.packages("devtools")
devtools::install_github("RockefellerUniversity/clipr")
```

### Installing rfastp... will be stable dependency once ClipR is released.
...
We can now use the CondaSysReqs package to create the required Conda environment and requirments for the clipr package.
...
CondaInfo <- install_CondaSysReqs("clipr")
...
### Using the CTK pipeline in clipr.

Once we have installed the requirements for CTK we can make use of the tools within clipr very easily.

First we load the clipR package and review the loading messages informing us of any established conda paths.

```{r CheckPaths,echo=TRUE,eval=TRUE}
library(clipR)
```

We can also list the important paths from the environmental options set by clipr.
...
Now that these are installed, all functions with the clipr package will use these by default without further configuration of paths or environmental variables.
...
require(clipR)
Fox3_Std <- system.file("extdata/Fox3_Std_small.fq.gz",package="clipR")
